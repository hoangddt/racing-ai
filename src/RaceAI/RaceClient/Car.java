...
public class Car {
	...
	private MyLogger _log = MyLogger.getInstance()
								.addNewLogger(getClass().getName(), FILE_LOG.Car);
	//Coordinate
	double x,y;
	//Direction
	double alpha;
	...
	public Car(double x, double y, double alpha, double s_x, double s_y){
		...
		String msg = "car constructor is called with";
		msg += " x: " + x;
		msg += " y: " + y;
		msg += " alpha: " + alpha;
		msg += " s_x: " + s_x;
	...
	{
		_log.writeLogMsg(FILE_LOG.Car, msg);
	}
	...
	void Update(double alpha, double x, double y,boolean flur){
		...
		this.y=y;
		this.flur = flur;
		this.writeLogMsg("Just update Car with (alpha, x, y, flur):" + alpha+ ", "+ x + ", "
				+ y +", " + flur);
	}
